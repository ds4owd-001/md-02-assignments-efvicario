...
5.  What are the data types of the three variables used for aesthetic mappings?
    ...
    **Answer: categorical (continent), continuous (life expectancy), discrete (count)**
...
#from https://www.geeksforgeeks.org/ignore-outliers-in-ggplot2-boxplot-in-r/
```
...
ggplot(data=gapminder_summary_continent_year,
  ...
  geom_line() +
  geom_point()
# from https://r-graph-gallery.com/connected_scatterplot_ggplot2.html
```
...
7.  What happens by adding the 'breaks' argument? Play around with the numbers inside of `seq()` to see what changes. Describe here what you observe:
    ...
    **This function controls how much of each axis is shown; the first numbers are the left-most and right-most line breaks, and the third is the width**
...
4.  What is stored in the `country_colors` object? Find out by executing the object in the Console (type it to the Console and hit enter). Do the same again, but with a question mark `?country_colors`. **Neat, it's pre-set colors for each country loaded to this dataset**
